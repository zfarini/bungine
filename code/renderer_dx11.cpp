...
Texture create_texture(String name, void *data, int width, int height,
                       bool srgb = true, bool mipmapping = true) {
  Texture result = {};
  ...
  platform.render_context->device->CreateTexture2D(&desc, 0, &texture);
  platform.render_context->context->UpdateSubresource(
      texture, 0, 0, s_data.pSysMem, s_data.SysMemPitch, 0);
  ...
  platform.render_context->device->CreateShaderResourceView(texture, &srv_desc,
                                                            &srv);
  if (mipmapping)
    platform.render_context->context->GenerateMips(srv);
...
void bind_texture(int index, Texture &texture) {
  platform.render_context->context->PSSetShaderResources(index, 1,
                                                         &texture.srv);
}
...
Shader load_shader(String filename, ShaderType type, const char *main = "") {
  ...
  MultiByteToWideChar(CP_ACP, 0, filename.data, (int)filename.count + 1,
  ...
  ID3DBlob *error_blob;
  if (FAILED(D3DCompileFromFile(wfilename, 0, D3D_COMPILE_STANDARD_FILE_INCLUDE,
                                main, version, flags, 0, &blob, &error_blob))) {
    OutputDebugStringA((char *)error_blob->GetBufferPointer());
    assert(0);
  }
  if (type == SHADER_TYPE_VERTEX)
    platform.render_context->device->CreateVertexShader(
        blob->GetBufferPointer(), blob->GetBufferSize(), 0, &result.vs);
  else if (type == SHADER_TYPE_FRAGMENT)
    platform.render_context->device->CreatePixelShader(
        blob->GetBufferPointer(), blob->GetBufferSize(), 0, &result.ps);
  else
    assert(0);
...
RasterizerState create_rasterizer_state(RasterizerFillMode fillmode,
  ...
  // NOTE: this seems to be for enabling depth clipping for the range specified
  // in viewport
  desc.DepthClipEnable = TRUE;
  platform.render_context->device->CreateRasterizerState(&desc, &result.state);
  return result;
}

RenderPass create_render_pass(Shader vs, Shader fs,
                              Array<VertexInputElement> input_elements,
  ...
  platform.render_context->device->CreateInputLayout(
      input_layout_desc, (UINT)input_elements.count,
      vs.blob->GetBufferPointer(), vs.blob->GetBufferSize(), &rp.input_layout);
...
DepthStencilState create_depth_stencil_state(bool enable_depth) {
  ...
  platform.render_context->device->CreateDepthStencilState(&desc,
                                                           &result.state);
  return result;
}
...
void begin_render_pass(RenderPass &rp) {
  ...
  if (rp.primitive_type == PRIMITIVE_TRIANGLES)
    platform.render_context->context->IASetPrimitiveTopology(
    ...
    assert(0);
  platform.render_context->context->IASetInputLayout(rp.input_layout);
  platform.render_context->context->VSSetShader(rp.vs.vs, 0, 0);
  platform.render_context->context->PSSetShader(rp.fs.ps, 0, 0);

  platform.render_context->context->OMSetDepthStencilState(
      rp.depth_stencil_state.state, 0);
  platform.render_context->context->RSSetState(rp.rasterizer_state.state);
}

void end_render_pass() { platform.render_context->render_pass = 0; }

void set_viewport(float top_left_x, float top_left_y, float width,
                  float height) {
  ...
  platform.render_context->context->RSSetViewports(1, &viewport);
}

void bind_framebuffer(FrameBuffer &framebuffer) {
  platform.render_context->context->OMSetRenderTargets(1, &framebuffer.rtv,
                                                       framebuffer.dsv);
}

void clear_framebuffer_color(FrameBuffer &framebuffer, v4 color) {
  platform.render_context->context->ClearRenderTargetView(framebuffer.rtv,
                                                          (FLOAT *)color.e);
}

void clear_framebuffer_depth(FrameBuffer &framebuffer, float depth) {
  platform.render_context->context->ClearDepthStencilView(
      framebuffer.dsv, D3D11_CLEAR_DEPTH, depth, 0);
}

VertexBuffer create_vertex_buffer(VertexBufferUsage usage, usize vertex_size,
                                  usize size, void *data = 0) {
  VertexBuffer result = {};
  ...
  if (!data) {
    ...
    platform.render_context->device->CreateBuffer(&desc, &init_data,
                                                  &result.buffer);
  }
  return result;
}
...
void update_vertex_buffer(VertexBuffer &vb, int size, void *data) {
  ...
  platform.render_context->context->Map(vb.buffer, 0, D3D11_MAP_WRITE_DISCARD,
                                        0, &buffer_mapped);
  memcpy(buffer_mapped.pData, data, size);
  platform.render_context->context->Unmap(vb.buffer, 0);
}

void bind_vertex_buffer(VertexBuffer &vb) {
  UINT stride = (UINT)vb.vertex_size;
  UINT offset = 0;
  platform.render_context->context->IASetVertexBuffers(0, 1, &vb.buffer,
                                                       &stride, &offset);
}

void draw(int offset, int vertices_count) {
  platform.render_context->context->Draw((UINT)vertices_count, (UINT)offset);
}

int get_type_alignement(ConstantBufferElementType type) { return 4; }
...
void update_constant_buffer(ConstantBuffer &buffer, void *data) {
  ...
  for (int i = 0; i < buffer.element_count; i++) {
    ...
    if (buffer.elements[i].array_size) {
      data_offset =
          align_to(data_offset, get_c_type_alignement(buffer.elements[i].type));
  ...
  D3D11_MAPPED_SUBRESOURCE buffer_mapped;
  platform.render_context->context->Map(
      buffer.buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &buffer_mapped);
...
void bind_constant_buffer(ConstantBuffer &cbuffer) {
  platform.render_context->context->VSSetConstantBuffers(0, 1, &cbuffer.buffer);
  platform.render_context->context->PSSetConstantBuffers(0, 1, &cbuffer.buffer);
}

void begin_render_frame() {
  RECT rect;
  GetClientRect(platform.render_context->window, &rect);
  int width = rect.right - rect.left;
  int height = rect.bottom - rect.top;

  if (width != platform.render_context->window_width ||
      height != platform.render_context->window_height) {
    platform.render_context->window_width = width;
    platform.render_context->window_height = height;

    platform.render_context->context->ClearState();
    platform.render_context->window_framebuffer.rtv->Release();
    platform.render_context->window_framebuffer.dsv->Release();

    platform.render_context->window_framebuffer.rtv = 0;
    platform.render_context->window_framebuffer.dsv = 0;

    if (width != 0 && height != 0) {
      if (FAILED(platform.render_context->swap_chain->ResizeBuffers(
              0, width, height, DXGI_FORMAT_UNKNOWN, 0)))
        assert(0);

      // create RenderTarget view for new backbuffer texture
      ID3D11Texture2D *backbuffer;
      platform.render_context->swap_chain->GetBuffer(
      ...
      platform.render_context->device->CreateRenderTargetView(
          (ID3D11Resource *)backbuffer, &backbuffer_desc,
          &platform.render_context->window_framebuffer.rtv);
      backbuffer->Release();
      ...
      platform.render_context->device->CreateTexture2D(&depth_buffer_desc, NULL,
                                                       &depth_buffer);
      platform.render_context->device->CreateDepthStencilView(
          (ID3D11Resource *)depth_buffer, NULL,
          &platform.render_context->window_framebuffer.dsv);
...
void end_render_frame() {
  ImGui::Render();
  platform.render_context->context->OMSetRenderTargets(
      1, &platform.render_context->window_framebuffer.rtv, 0);
  ImGui_ImplDX11_RenderDrawData(ImGui::GetDrawData());
  platform.render_context->swap_chain->Present(1, 0);
}
...
void init_render_context_dx11(RenderContext &rc, HWND window) {
...

  {
    ...
    assert(SUCCEEDED(D3D11CreateDeviceAndSwapChain(
        0, D3D_DRIVER_TYPE_HARDWARE, 0, flags, 0, 0, D3D11_SDK_VERSION, &desc,
        &rc.swap_chain, &rc.device, 0, &rc.context)));
  }
#ifdef RENDERER_DEBUG
  {
...

  {
    ...
    ID3D11Texture2D *backbuffer;
    rc.swap_chain->GetBuffer(0, __uuidof(ID3D11Texture2D),
                             (void **)&backbuffer);
    D3D11_RENDER_TARGET_VIEW_DESC backbuffer_desc = {};
    backbuffer_desc.Format = DXGI_FORMAT_R8G8B8A8_UNORM_SRGB;
    backbuffer_desc.ViewDimension = D3D11_RTV_DIMENSION_TEXTURE2D;
    rc.device->CreateRenderTargetView((ID3D11Resource *)backbuffer,
                                      &backbuffer_desc,
    ...
    rc.device->CreateDepthStencilView(depth_buffer, 0,
                                      &rc.window_framebuffer.dsv);
