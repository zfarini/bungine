#define EDITOR_ROTATION_INNER_RADIUS 1.5f
#define EDITOR_ROTATION_OUTER_RADIUS 2
...
void update_editor(Game &game, World &world, Editor &editor, GameInput &input, Camera &camera)
{
	...
	if (IsDown(input, BUTTON_LEFT_CONTROL)) {
		if (editor.mode == EDITOR_MODE_TRANSLATION || editor.mode == EDITOR_MODE_ROTATION
				|| editor.mode == EDITOR_MODE_SCALE) {
			if (IsDown(input, BUTTON_T)) editor.mode = EDITOR_MODE_TRANSLATION;
			else if (IsDown(input, BUTTON_R)) editor.mode = EDITOR_MODE_ROTATION;
			else if (IsDown(input, BUTTON_S)) editor.mode = EDITOR_MODE_SCALE;
		}
		
		if (IsDownFirstTime(input, BUTTON_Q))
			editor.mode = EDITOR_MODE_COLLISION_MESH;
	}

	if (editor.mode == EDITOR_MODE_TRANSLATION || editor.mode == EDITOR_MODE_ROTATION
			|| editor.mode == EDITOR_MODE_SCALE)
	{
		// @CLEANUP: why length = 2? do we even use it?
			...
			if (editor.in_gizmo) {
				...
				if (e) {
					...
					if (editor.gizmo_mode == GIZMO_SCALE && editor.uniform_scale) {
						int best = 0;
						for (int i = 1; i < 3; i++) {
							if (fabsf(dot(axis[i], camera.forward)) < fabsf(dot(axis[best], camera.forward)))
								best = i;
						}
						editor.dragging_axis = best;
					}

					{
						//if (fabsf(dot(axis[editor.dragging_axis], camera.forward)) > 0.9) {
						//	if (fabsf(dot(axis[(editor.dragging_axis+1)%3], camera.forward)) > fabsf(dot(axis[(editor.dragging_axis+2)%3], camera.forward)))
						//		plane_normal = axis[(editor.dragging_axis+1)%3];
						//	else
						//		plane_normal = axis[(editor.dragging_axis+2)%3];
						//}
						//else
						plane_normal = cross(axis[editor.dragging_axis], cross(axis[editor.dragging_axis], camera.forward));

						v3 N[4];
						N[0] = normalize(plane_normal);
						N[1] = normalize(camera.forward + axis[editor.dragging_axis]);
						N[2] = normalize(camera.forward - axis[editor.dragging_axis]);
						N[3] = normalize(plane_normal);


						float c[4];
						for (int i = 0; i < 4; i++)
							c[i] = fabsf(dot(N[i], camera.forward)) - 
								fabsf(dot(N[i], axis[editor.dragging_axis]));
						for (int i = 0; i < 4; i++)
							printf("%f ", c[i]);
						printf("\n");
